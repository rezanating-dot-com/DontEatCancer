"""Seed the database with ~100 common food additives."""
...
SEED_DATA = [
    ...
    {"name": "High Fructose Corn Syrup", "cas": None, "e": None, "cat": "sweetener",
    ...
    {"name": "Modified Starch", "cas": None, "e": "E1404", "cat": "thickener",
    ...
    {"name": "Potassium Bromate", "cas": "7758-01-2", "e": None, "cat": "flour treatment",
    ...
    {"name": "Brominated Vegetable Oil", "cas": "8016-94-2", "e": None, "cat": "emulsifier",
    ...
    {"name": "Sodium Lauryl Sulfate", "cas": "151-21-3", "e": None, "cat": "emulsifier",
...
def seed():
    ...
    from app.models import evidence  # noqa: F401 — register all models
    ...
    for item in SEED_DATA:
        ...
        existing = db.query(Ingredient).filter(Ingredient.slug == slug).first()
        if existing:
            skipped += 1
            continue
        ...
        ingredient = Ingredient(
            ...
            overall_risk_level="insufficient",
            evidence_count=0,
        )
        db.add(ingredient)
        db.flush()
        ...
        # Translated aliases
        for alias_name, lang in item.get("aliases", []):
            db.add(IngredientAlias(
                ingredient_id=ingredient.id,
